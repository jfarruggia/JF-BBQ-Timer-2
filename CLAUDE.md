...
- **Deployment target:** iOS app **16.6** (per `project.pbxproj`; the shipped 1.2.2 App Store build was 15.6, so V2 drops iOS 15 users — they stay on 1.2.2), watchOS 11.6; builds against the iOS/watchOS 26 SDK. (Test/UITest targets are at 18.2.) Because the app reaches back to iOS 16.6, **iOS 17+ APIs need gating**, and **iOS 26 Liquid Glass code is gated behind `if #available(iOS 26, *)` with a non-glass fallback in the `else`** (see the timer cards / `TimerContainerAppearance`, and the shared glass helpers in `ButtonStyles.swift`).
